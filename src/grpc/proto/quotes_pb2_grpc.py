# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc
import warnings

from . import quotes_pb2 as quotes__pb2

GRPC_GENERATED_VERSION = '1.65.0'
GRPC_VERSION = grpc.__version__
EXPECTED_ERROR_RELEASE = '1.65.0'
SCHEDULED_RELEASE_DATE = 'June 25, 2024'
_version_not_supported = False

try:
    from grpc._utilities import first_version_is_lower
    _version_not_supported = first_version_is_lower(GRPC_VERSION, GRPC_GENERATED_VERSION)
except ImportError:
    _version_not_supported = True

if _version_not_supported:
    warnings.warn(
        f'The grpc package installed is at version {GRPC_VERSION},'
        + ' but the generated code in quotes_pb2_grpc.py depends on'
        + f' grpcio>={GRPC_GENERATED_VERSION}.'
        + f' Please upgrade your grpc module to grpcio>={GRPC_GENERATED_VERSION}'
        + f' or downgrade your generated code using grpcio-tools<={GRPC_VERSION}.'
        + f' This warning will become an error in {EXPECTED_ERROR_RELEASE},'
        + f' scheduled for release on {SCHEDULED_RELEASE_DATE}.',
        RuntimeWarning
    )


class QuotesStub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.GetQuoteHandler = channel.unary_unary(
                '/Quotes/GetQuoteHandler',
                request_serializer=quotes__pb2.UserIDRequest.SerializeToString,
                response_deserializer=quotes__pb2.Quote.FromString,
                _registered_method=True)
        self.GetSameQuoteHandler = channel.unary_unary(
                '/Quotes/GetSameQuoteHandler',
                request_serializer=quotes__pb2.UserAndQuoteIDRequest.SerializeToString,
                response_deserializer=quotes__pb2.Quote.FromString,
                _registered_method=True)
        self.LikeQuoteHandler = channel.unary_unary(
                '/Quotes/LikeQuoteHandler',
                request_serializer=quotes__pb2.UserAndQuoteIDRequest.SerializeToString,
                response_deserializer=quotes__pb2.Empty.FromString,
                _registered_method=True)


class QuotesServicer(object):
    """Missing associated documentation comment in .proto file."""

    def GetQuoteHandler(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def GetSameQuoteHandler(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def LikeQuoteHandler(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_QuotesServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'GetQuoteHandler': grpc.unary_unary_rpc_method_handler(
                    servicer.GetQuoteHandler,
                    request_deserializer=quotes__pb2.UserIDRequest.FromString,
                    response_serializer=quotes__pb2.Quote.SerializeToString,
            ),
            'GetSameQuoteHandler': grpc.unary_unary_rpc_method_handler(
                    servicer.GetSameQuoteHandler,
                    request_deserializer=quotes__pb2.UserAndQuoteIDRequest.FromString,
                    response_serializer=quotes__pb2.Quote.SerializeToString,
            ),
            'LikeQuoteHandler': grpc.unary_unary_rpc_method_handler(
                    servicer.LikeQuoteHandler,
                    request_deserializer=quotes__pb2.UserAndQuoteIDRequest.FromString,
                    response_serializer=quotes__pb2.Empty.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'Quotes', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))
    server.add_registered_method_handlers('Quotes', rpc_method_handlers)


 # This class is part of an EXPERIMENTAL API.
class Quotes(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def GetQuoteHandler(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/Quotes/GetQuoteHandler',
            quotes__pb2.UserIDRequest.SerializeToString,
            quotes__pb2.Quote.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def GetSameQuoteHandler(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/Quotes/GetSameQuoteHandler',
            quotes__pb2.UserAndQuoteIDRequest.SerializeToString,
            quotes__pb2.Quote.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def LikeQuoteHandler(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/Quotes/LikeQuoteHandler',
            quotes__pb2.UserAndQuoteIDRequest.SerializeToString,
            quotes__pb2.Empty.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)
